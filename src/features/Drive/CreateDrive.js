...
import { validateCreateDrive } from "../../validations/DriveValidation";
...
function CreateDrive() {
  ...
  const addDrive = async () => {
    if (validateCreateDrive(driveData)) {
      try {
        ...
        dispatch(
          showNotification({ message: "Failed to add drive", status: 0 })
        );
      }
    } else {
      dispatch(
  ...
  return (
    <>
      <TitleCard title="New Drive" topMargin="mt-2">
        ...
        <div className="mt-16">
          <button className="btn btn-primary float-right" onClick={addDrive}>
            Create
          </button>
        </div>
      </TitleCard>
